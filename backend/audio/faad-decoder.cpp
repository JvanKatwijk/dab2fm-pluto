...
#include        "neaacdec.h"
        ...
        faadDecoder::faadDecoder	(RingBuffer<std::complex<int16_t>> *pcmBuffer,
...
//      fprintf (stderr, "header = %d\n", hInfo. header_type);
        ...
        if (channels == 2) {
           output (outBuffer, samples, sp -> aacChannelMode ||
	                               sp -> psFlag, sampleRate);
        }
        else
        if (channels == 1) {
	   ...
	   output (buffer, 2 * samples, sp -> aacChannelMode ||
	                                sp -> psFlag, sampleRate);
        }
        else
...
void	faadDecoder::output (int16_t *buffer, int     size,
	...
	if (the_callBacks -> audioOutHandler == NULL)
	   return;
	pcmBuffer	-> putDataIntoBuffer ((std::complex<float> *)buffer,
	                                      size / 2);
	the_callBacks -> audioOutHandler (rate, userData);
}
